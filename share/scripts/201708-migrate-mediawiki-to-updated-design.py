...
parser.add_argument('-d', '--old-db', nargs=1, help='old database name', metavar='database name', dest='olddb')
...
if args.olddb:
	...
	parser.error('Provide the database name where the old mediawiki data lives.')
...
OLDDB = Database(config["db_host"],config["db_user"],config["db_passwd"],olddb)
